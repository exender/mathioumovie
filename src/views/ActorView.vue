    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div v-if="selectedActor">
        <h1 class="text-2xl font-bold mb-4">
          Filmographie de {{ selectedActor.name }}
        </h1>
        <div class="flex items-center mb-4">
          <img
            v-if="selectedActor.profile_path"
            :src="
          ...
          <svg
            v-else
            class="w-16 h-16 rounded-full mr-4 text-gray-500"
...
export default {
  ...
  mounted() {
    ...
    axios
      .get(
        `https://api.themoviedb.org/3/person/${this.$route.params.id}?api_key=75e981bcdd819c45eea5057ee60c7c36`
      )
      .then((response) => {
        ...
        axios
          .get(
            `https://api.themoviedb.org/3/person/${this.selectedActor.id}/movie_credits?api_key=75e981bcdd819c45eea5057ee60c7c36`
          )
          .then((response) => {
